	...
	touch.on("#fullpage","swipe",function(e){
		if(e.direction=="up"){
			...
			$(".section").each(function(index,obj){
				if(index==n){
					$(obj).find(".boxLeft").css({"transform":"translate(0,0)"});
					$(obj).find(".tu").css({"transform":"translate(0,0)"});
				}else{
					$(obj).find(".boxLeft").css({"transform":"translate(-50px,0)"});
					$(obj).find(".tu").css({"transform":"translate(50px,0)"});
				}
			});
			$(".yuanquan").css("background","transparent").eq(n).css("background","#333");
